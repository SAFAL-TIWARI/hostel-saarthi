                ...
                <main className="flex-1 overflow-x-hidden overflow-y-auto bg-slate-50 p-4 lg:p-8 dark:bg-slate-950">
                    ...
                    <div className="max-w-7xl mx-auto space-y-6">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                <h2 className="text-2xl font-bold font-display text-slate-800 dark:text-white">Lost & Found Hub 🕵️‍♂️</h2>
                                <p className="text-slate-500 dark:text-slate-400">Report lost items or help others find theirs.</p>
                            </div>
                        ...
                        {/* Filters & Search */}
                        <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between dark:bg-slate-900 dark:border-slate-800">
                            ...
                            <div className="relative w-full md:w-64">
                                ...
                                <input
                                    ...
                                    className="w-full pl-9 pr-4 py-2 bg-slate-50 border-none rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/20 focus:outline-none dark:bg-slate-800 dark:text-white dark:placeholder-slate-500"
                                />
                            </div>
                        </div>
                                ...
                                <Card key={item.id} hover className="overflow-hidden group">
                                    <div className="h-48 bg-slate-100 relative overflow-hidden">
                                        {item.image ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center bg-slate-100 text-slate-300 dark:bg-slate-800 dark:text-slate-600">
                                    ...
                                    <CardContent className="p-4">
                                        <div className="flex justify-between items-start mb-2">
                                            <h3 className="font-bold text-lg text-slate-800 line-clamp-1 dark:text-slate-100">{item.title}</h3>
                                            <span className="text-xs text-slate-400 whitespace-nowrap flex items-center dark:text-slate-500">
                                                <Clock className="w-3 h-3 mr-1" />
                                                {new Date(item.date).toLocaleDateString()}
                                            </span>
                                        </div>
                                        <p className="text-sm text-slate-600 line-clamp-2 mb-4 h-10 dark:text-slate-400">{item.description}</p>
                                            ...
                                            {isWarden ? (
                                                ...
                                                <>
                                                    ...
                                                    {(item.status === 'Claimed' || item.status === 'Returned') && (
                                                        <div className="w-full text-center py-1.5 text-xs font-medium text-emerald-600 bg-emerald-50 rounded-lg flex items-center justify-center gap-1 dark:bg-emerald-900/20 dark:text-emerald-400">
                ...
                <form onSubmit={handleAddItem} className="space-y-4">
                    <div className="flex gap-4">
                        <label className="flex-1 cursor-pointer">
                            ...
                            />
                            <div className="p-3 text-center rounded-xl border-2 border-slate-100 peer-checked:border-rose-500 peer-checked:bg-rose-50 peer-checked:text-rose-700 font-bold text-slate-500 transition-all dark:border-slate-700 dark:text-slate-400 dark:peer-checked:bg-rose-900/20 dark:peer-checked:text-rose-400">
                                LOST
                            </div>
                        </label>
                        <label className="flex-1 cursor-pointer">
                            ...
                            />
                            <div className="p-3 text-center rounded-xl border-2 border-slate-100 peer-checked:border-emerald-500 peer-checked:bg-emerald-50 peer-checked:text-emerald-700 font-bold text-slate-500 transition-all dark:border-slate-700 dark:text-slate-400 dark:peer-checked:bg-emerald-900/20 dark:peer-checked:text-emerald-400">
                                FOUND
                            </div>
                        </label>
                    </div>
                    ...
                    <div className="space-y-1.5">
                        ...
                        <textarea
                            className="w-full px-4 py-2 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 min-h-[80px] dark:bg-slate-800 dark:border-slate-700 dark:text-white dark:placeholder-slate-500"
